...
const translations = {
  en: {
      ...
      lingo:"English language learning mobile app using Android+Java LittleLingo is an innovative app designed to enhance English language learning through interactive and engaging methods. It features comprehensive vocabulary and grammar lessons, audio support for pronunciation, quizzes, and progress tracking for learners, parents, and teachers. Developed using Java for Android, the app integrates Firebase for authentication and data storage, and utilizes APIs for enhanced functionality. LittleLingo aims to improve English proficiency while making learning",
      nutri:"NutriScan is a comprehensive nutrition and health app designed to manage dietary needs by empowering users to make informed decisions and improve overall well-being. It addresses key challenges by offering personalized calorie recommendations, healthy recipes, and food scanning for balanced diet tracking. The app also calculates BMI and BMR based on weight, height, age, and daily activity level. From a business perspective, NutriScan simplifies the path to a healthier lifestyle by providing a variety of meal recipes with nutritional facts, and features for searching and filtering recipes.",
      foodi:"The FOODi FOODi project, developed by the WebWiz Group, aimed to create an aesthetically pleasing and user-friendly restaurant website, incorporating best practices in UI design. The website features online reservation capabilities, order and cart management, and an admin platform for managing menus and users. The database is structured to track users, orders, reservations, and menu items with One-to-Many relationships and ON DELETE CASCADE constraints. The site employs APIs for restaurant location mapping, order timestamps, and random cocktail generation. Security is enhanced with hashed passwords and strict password requirements. Challenges included database updates, JSON data handling with AJAX, and page redesign."
  },
  fr: {
      about: "À propos",
      ...
      lingo:"LittleLingo est une application innovante conçue pour améliorer l'apprentissage de l'anglais grâce à des méthodes interactives et engageantes. Elle propose des leçons complètes de vocabulaire et de grammaire, un support audio pour la prononciation, des quiz et un suivi des progrès pour les apprenants, les parents et les enseignants. Développée en Java pour Android, l'application intègre Firebase pour l'authentification et le stockage des données, et utilise des API pour des fonctionnalités améliorées. LittleLingo vise à améliorer la maîtrise de l'anglais tout en rendant l'apprentissage agréable.",
      nutri:"NutriScan est une application complète de nutrition et de santé conçue pour gérer les besoins alimentaires en permettant aux utilisateurs de prendre des décisions éclairées et d'améliorer leur bien-être général. Elle résout les problèmes en offrant des recommandations de calories personnalisées, des recettes saines et une fonction de scan des aliments pour suivre un régime équilibré. L'application calcule également l'IMC et le BMR en fonction du poids, de la taille, de l'âge et du niveau d'activité quotidienne. Du point de vue commercial, NutriScan simplifie le chemin vers un mode de vie plus sain en fournissant une variété de recettes de repas avec des informations nutritionnelles, ainsi que des fonctionnalités de recherche et de filtrage des recettes",
      foodi:"Le projet FOODi FOODi, développé par le groupe WebWiz, a visé à créer un site web de restaurant esthétiquement plaisant et convivial, intégrant les meilleures pratiques de design UI. Il propose des fonctionnalités telles que la réservation en ligne, la gestion des commandes et des paniers, ainsi qu'un système de gestion administrateur pour les menus et les utilisateurs. La base de données est conçue pour suivre les utilisateurs, les commandes, les réservations et les éléments de menu, avec des relations One-to-Many et des contraintes ON DELETE CASCADE. Le site utilise des API pour localiser le restaurant, afficher des horodatages des commandes, et générer des cocktails aléatoires. La sécurité est renforcée avec des mots de passe hachés et des contraintes de mot de passe strictes. Les défis rencontrés incluaient la mise à jour des informations dans la base de données, la gestion des données JSON avec AJAX, et la refonte de l'apparence des pages."
  },
};
